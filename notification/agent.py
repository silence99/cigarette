...
class Agent(asyncio.Protocol):
    ...
    def data_received(self, data):   
        try:     
            ...
            if uid not in self.staged:
                ...
                record = Record(uid, ip=self.ip, port=self.port, lenRecv=len(data)) 
            ...
            self.logger.error("received invalid message: %s" % data.decode('utf-8'))
    ...
    def connection_lost(self, exc):
        ...
        self.logger.warn("not all notification completed when connection close")
    ...
    def SendNotification(self, config):
        if config is None:
            self.logger.warn("the configuration notification contains none message")
            return
        ...
        record = Record(uid, ip=self.ip, port=self.port, startSend=time.time(), lenSend=len(msgSend))
